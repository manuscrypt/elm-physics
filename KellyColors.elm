...
randomColors : Seed -> Int -> ( List Color, Seed )
randomColors seed howMany =
    let
        max =
            (List.length kellyColors) - 1

        ( idxs, seed0 ) =
            step (intList 0 max howMany) seed

        kellys =
            List.map (\i -> Maybe.withDefault (KellyDef 267 "222222" 34 34 34) (List.drop i kellyColors |> List.head)) idxs
    in
        ( List.map toColor kellys, seed0 )
